...
library(MPRAnalyze)
# Produce example data: lognormal distributed DNA and poisson distributed RNA
sim <- simulateMPRA(n.case = 30, n.ctrl = 0, n.reps = 3,  
                    sd.dna.cond = 0.1, sd.dna.bc = 0.05, 
                    sd.rna = 1, sd.rna.cond = 0.5)
# Run MPRAnalyze
ctrls <- which(sim$rowAnnot$type == "ctrl")
...
abline(a = 0, b = 1, col="red")
plot(mpraobj@rnaCounts["case_1",]/mpraobj@dnaCounts["case_1",], 
     getRNAFits(mpraobj, enhancers=c("case_1"), full=TRUE) /
         getDNAFits(mpraobj, enhancers=c("case_1"), full=TRUE))
abline(a = 0, b = 1, col="red")
```
...
library(MPRAnalyze)
# Produce example data: lognormal distributed DNA and poisson distributed RNA
sim <- simulateMPRA(n.case = 20, n.ctrl = 20, n.reps = 2, 
                    sd.dna.cond = 0.1, sd.dna.bc = 0.05, 
                    sd.rna = 1, sd.rna.cond = 0.5)
# Run MPRAnalyze
ctrls <- which(sim$rowAnnot$type == "ctrl")
...
library(MPRAnalyze)
# Produce example data: lognormal distributed DNA and poisson distributed RNA
sim <- simulateMPRA(n.case = 2, n.ctrl = 5, n.reps = 2, 
                    sd.dna.cond = 0.1, sd.dna.bc = 0.05, 
                    sd.rna = 1, sd.rna.cond = 0.5)
# Run MPRAnalyze
ctrls <- which(sim$rowAnnot$type == "ctrl")
