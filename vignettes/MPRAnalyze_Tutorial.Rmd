...
The general idea is that most questions below can be answered with a hypothesis test:
We chose a log-likelihood ratio test to compare a full and a reduced model.
...
strIDToPlot <- rownames(objMPRA$dfMPRAnalyzeResults)[order(objMPRA$dfMPRAnalyzeResults$padj)[1]]
gplotBoxplots <- plotBoxplots(objMPRA,strIDToPlot)
print(gplotBoxplots)
```
