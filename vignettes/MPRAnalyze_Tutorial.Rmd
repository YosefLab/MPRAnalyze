...
library(MPRAnalyze)
detach("package:MPRAnalyze", unload=TRUE)
library(MPRAnalyze)
# ln.nb: does not work
# gamma.pois: yes
# Produce example data: lognormal distributed DNA and poisson distributed RNA
...
sim <- simulateMPRA(
    model="gamma.pois", n.case=10, n.ctrl=4, frac.de=0.5, 
    n.cond=1, n.bc=25, n.reps=3, 
    mu.dna=100, mu.sd.dna=0.3, sd.dna=10, sd.dna.cond=0, sd.dna.bc=0.2,
    mu.rna=1, mu.sd.rna=0.1, sd.rna=NULL, sd.rna.cond=0, sd.rna.cond_case=0.1)

mpraobj <- MpraObject(dnaCounts=sim$dcounts.obs, rnaCounts=sim$rcounts.obs,
                      colAnnot=sim$colAnnot, controls=which(sim$rowAnnot=="ctrl"),
                      BPPARAM=SerialParam())
mpraobj <- estimateDepthFactors(mpraobj, lib.factor="sample", which.lib="both")
mpraobj <- setModel(obj = mpraobj, model = "gamma.pois")
mpraobj <- analyze.quantitative.lrt.NEW(
    obj = mpraobj, mode = "scaled", dnaDesign = ~barcode)
mpraobj <- test.lrt(mpraobj)
print(head(mpraobj@results))
```
...
library(BiocParallel)
sim <- simulateMPRA(
    model="ln.ln", n.case=10, n.ctrl=4, frac.de=0.5, 
    n.cond=2, n.bc=25, n.reps=3, 
    mu.dna=100, mu.sd.dna=0.3, sd.dna=10, sd.dna.cond=0.2, sd.dna.bc=0.2,
    mu.rna=1, mu.sd.rna=0.1, sd.rna=1, sd.rna.cond=0.1, sd.rna.cond_case=0.1)
mpraobj <- MpraObject(dnaCounts=sim$dcounts.obs, rnaCounts=sim$rcounts.obs,
                      colAnnot=sim$colAnnot, controls=which(sim$rowAnnot=="ctrl"),
                      BPPARAM=SerialParam())
mpraobj <- estimateDepthFactors(mpraobj, lib.factor="sample", which.lib="both")
mpraobj <- setModel(obj = mpraobj, model = "ln.ln")
mpraobj <- analyze.comparative.lrt(
    obj = mpraobj, condition = "cond", mode = "scaled", 
    dnaDesign = ~barcode+cond, rnaDesign = ~cond)
mpraobj <- test.lrt(mpraobj)
```
...
mpraobj <- setModel(obj = mpraobj, model = "ln.ln")
mpraobj <- analyze.comparative.lrt(
    obj = mpraobj, condition = "cond", mode = "full", 
    dnaDesign = ~barcode+cond, rnaDesign = ~cond)
mpraobj <- test.lrt(mpraobj)
```
...
```{r }
plot(sim$rowAnnot$cond2_effect, log(mpraobj@results$pval))
plot(mpraobj@dnaCounts["case_2",], getDNAFits(mpraobj, enhancers=c("case_2"), full=TRUE))
abline(a = 0, b = 1, col="red")
plot(mpraobj@rnaCounts["case_3",], getRNAFits(mpraobj, enhancers=c("case_3"), full=TRUE))
abline(a = 0, b = 1, col="red")
plotBoxplots(mpraobj, "case_1", condition = "cond", batch = "rep")
plotBoxplots(mpraobj, "case_2", condition = "cond", batch = "rep")
plotBoxplots(mpraobj, "case_4", condition = "cond", batch = "rep")
plotBoxplots(mpraobj, "case_6", condition = "cond", batch = "rep")
```
