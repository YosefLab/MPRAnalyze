...
# Produce example data: lognormal distributed DNA and poisson distributed RNA
sim <- simulateMPRA(
    model="ln.ln", n.case=20, n.ctrl=5, frac.de=0.5, 
    n.cond=2, n.bc=5, n.reps=2, 
...
mpraobj <- MpraObject(
    dnaCounts=sim$dcounts.obs, rnaCounts=sim$rcounts.obs,
    colAnnot=sim$colAnnot, controls=ctrls,
    BPPARAM=SerialParam())
...
mpraobj <- MpraObject(dnaCounts=sim$dcounts.obs, rnaCounts=sim$rcounts.obs,
                      dnaAnnot=sim$colAnnot, rnaAnnot=sim$colAnnot,
                      controls=ctrls, BPPARAM = SerialParam())
mpraobj <- estimateDepthFactors(mpraobj,  lib.factor="sample", which.lib="both")
mpraobj <- setModel(mpraobj, "ln.ln")
mpraobj <- analyze.comparative.lrt(
